...
const generatePatternProbabilities = (): PatternProbability[] => {
  const patterns = [
    ...
    'Morning Star', 'Evening Star', 'Doji', 'Harami'
  ];
...
export function AIAnalysisDashboard({ symbol = 'NIFTY', currentPrice = 19850 }: AIAnalysisDashboardProps) {
    ...
    <div className="space-y-6">
          ...
          <CardContent>
            <div className="grid gap-4 md:grid-cols-5">
              <div className="text-center">
                ...
                <div className={cn("text-2xl font-bold", getSentimentColor(sentiment.overall))}>
                  {sentiment.overall > 0 ? '+' : ''}{sentiment.overall.toFixed(0)}
                </div>
                <Progress value={Math.abs(sentiment.overall) * 2} className="mt-2" />
              </div>
              
              <div className="text-center">
                <div className="text-sm font-medium mb-2">News</div>
                <div className={cn("text-xl font-bold", getSentimentColor(sentiment.news))}>
                  {sentiment.news > 0 ? '+' : ''}{sentiment.news.toFixed(0)}
                </div>
                <Progress value={Math.abs(sentiment.news) * 1.5} className="mt-2" />
              </div>
              
              <div className="text-center">
                <div className="text-sm font-medium mb-2">Social</div>
                <div className={cn("text-xl font-bold", getSentimentColor(sentiment.social))}>
                  {sentiment.social > 0 ? '+' : ''}{sentiment.social.toFixed(0)}
                </div>
              ...
              <div className="text-center">
                ...
                <div className={cn("text-xl font-bold", getSentimentColor(sentiment.technical))}>
                  {sentiment.technical > 0 ? '+' : ''}{sentiment.technical.toFixed(0)}
                </div>
                <Progress value={Math.abs(sentiment.technical) * 2} className="mt-2" />
              </div>
              
              <div className="text-center">
                <div className="text-sm font-medium mb-2">Options</div>
                <div className={cn("text-xl font-bold", getSentimentColor(sentiment.options))}>
                  {sentiment.options > 0 ? '+' : ''}{sentiment.options.toFixed(0)}
                </div>
                <Progress value={Math.abs(sentiment.options) * 1.2} className="mt-2" />
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Machine learning models forecasting price movements with confidence levels
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            {predictions.map((model, index) => (
              <div key={index} className="p-4 border rounded-lg">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-sm text-muted-foreground">{model.timeframe}</span>
                    <Badge
                      variant={
                        model.prediction === 'bullish' ? 'default' :
                        model.prediction === 'bearish' ? 'destructive' :
                        'secondary'
                      }
                    >
                ...
                <div className="grid gap-4 md:grid-cols-3 mb-3">
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <div className="text-lg font-bold">{model.confidence.toFixed(0)}%</div>
                  ...
                  {model.target && (
                    <div>
                      ...
                      <div className="text-lg font-mono">₹{model.target.toFixed(0)}</div>
                    </div>
                  )}
                  <div>
                    ...
                    <div className={cn(
                      "text-lg font-bold",
                      model.target && model.target > currentPrice ? "text-trading-bull" : 
                      model.target && model.target < currentPrice ? "text-trading-bear" : 
                      "text-trading-neutral"
                    )}>
                      {model.target ? `${((model.target - currentPrice) / currentPrice * 100).toFixed(1)}%` : 'N/A'}
                    </div>
                  </div>
                </div>
                
                <div>
                  <div className="text-sm font-medium mb-2">Model Reasoning:</div>
              ...
              <div key={index} className="p-3 border rounded-lg">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-mono">{pattern.probability.toFixed(0)}%</span>
                  </div>
                  <Progress value={pattern.probability} />
                  
                  <div className="flex justify-between text-sm">
                    <span>Historical Accuracy</span>
                    <span className="font-mono">{pattern.accuracy.toFixed(0)}%</span>
                  </div>
                </div>
