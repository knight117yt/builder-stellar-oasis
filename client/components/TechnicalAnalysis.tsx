...
const calculateSupportResistance = (currentPrice: number): SupportResistance[] => {
...
const calculateTechnicalIndicators = (currentPrice: number): TechnicalIndicator[] => {
  return [
    {
      name: 'RSI (14)',
...
export function TechnicalAnalysis({ symbol = 'NIFTY', currentPrice = 19850 }: TechnicalAnalysisProps) {
  ...
  const nearestResistance = supportResistance.find(sr => sr.type === 'resistance');
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid gap-4 md:grid-cols-2">
            <div>
              <h4 className="font-medium mb-3 text-trading-bull">Support Levels</h4>
              <div className="space-y-2">
                {supportResistance.filter(sr => sr.type === 'support').slice(0, 3).map((level, index) => (
                  <div key={index} className="flex items-center justify-between p-2 bg-trading-bull-light rounded">
            ...
            <div>
              <h4 className="font-medium mb-3 text-trading-bear">Resistance Levels</h4>
              <div className="space-y-2">
                {supportResistance.filter(sr => sr.type === 'resistance').slice(0, 3).map((level, index) => (
                  <div key={index} className="flex items-center justify-between p-2 bg-trading-bear-light rounded">
                    <div className="flex items-center gap-2">
          ...
          <CardContent>
            <div className="grid gap-4 md:grid-cols-3 lg:grid-cols-7">
              <div className="text-center">
                ...
                <div className="font-mono text-sm text-trading-bear">₹{cprLevels.r2.toFixed(0)}</div>
              </div>
              <div className="text-center">
                <div className="text-xs text-muted-foreground">R1</div>
                <div className="font-mono text-sm text-trading-bear">₹{cprLevels.r1.toFixed(0)}</div>
              </div>
              <div className="text-center">
                <div className="text-xs text-muted-foreground">TC</div>
                <div className="font-mono text-sm font-bold">₹{cprLevels.tc.toFixed(0)}</div>
              </div>
              <div className="text-center">
                <div className="text-xs text-muted-foreground">PIVOT</div>
                <div className="font-mono text-sm font-bold text-primary">₹{cprLevels.pivot.toFixed(0)}</div>
              </div>
              <div className="text-center">
                <div className="text-xs text-muted-foreground">BC</div>
                <div className="font-mono text-sm font-bold">₹{cprLevels.bc.toFixed(0)}</div>
              </div>
              <div className="text-center">
                <div className="text-xs text-muted-foreground">S1</div>
                <div className="font-mono text-sm text-trading-bull">₹{cprLevels.s1.toFixed(0)}</div>
              </div>
              <div className="text-center">
                <div className="text-xs text-muted-foreground">S2</div>
                <div className="font-mono text-sm text-trading-bull">₹{cprLevels.s2.toFixed(0)}</div>
              </div>
            </div>
            
            <div className="mt-4 p-3 bg-muted rounded-lg">
              <div className="text-sm font-medium mb-1">CPR Width: ₹{Math.abs(cprLevels.tc - cprLevels.bc).toFixed(0)}</div>
              <div className="text-xs text-muted-foreground">
                {Math.abs(cprLevels.tc - cprLevels.bc) < 50 ? 'Narrow CPR - Trending day expected' : 'Wide CPR - Sideways movement likely'}
              </div>
            </div>
          </CardContent>
        </Card>
              ...
              <div key={index} className="p-3 border rounded-lg">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <Badge
                    ...
                    className="text-xs"
                  >
                    {indicator.signal === 'bullish' && <ArrowUp className="h-3 w-3 mr-1" />}
                    {indicator.signal === 'bearish' && <ArrowDown className="h-3 w-3 mr-1" />}
                    {indicator.signal === 'neutral' && <Minus className="h-3 w-3 mr-1" />}
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
              <div className="text-center p-3 border rounded-lg">
                ...
                {oiAnalysis.callUnwinding && (
                  <Badge variant="outline" className="text-xs mt-1">Unwinding</Badge>
                )}
              </div>
              ...
              <div className="text-center p-3 border rounded-lg">
                ...
                <div className={cn(
                  "text-lg font-bold",
                  oiAnalysis.pcr > 1.2 ? "text-trading-bull" :
                  oiAnalysis.pcr < 0.8 ? "text-trading-bear" :
                  "text-trading-neutral"
                )}>
                  {oiAnalysis.pcr.toFixed(2)}
                </div>
                <div className="text-xs text-muted-foreground">
                  {oiAnalysis.pcr > 1.2 ? 'Bullish' : oiAnalysis.pcr < 0.8 ? 'Bearish' : 'Neutral'}
                </div>
              </div>
              ...
              <div className="text-center p-3 border rounded-lg">
                ...
                <div className="text-xs text-muted-foreground">
                  {Math.abs(currentPrice - oiAnalysis.maxPainStrike).toFixed(0)} pts away
                </div>
              </div>
            </div>
          </CardContent>
