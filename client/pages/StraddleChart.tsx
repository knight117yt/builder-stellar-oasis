...
export default function StraddleChart() {
  ...
  const generateMockStraddleData = (symbol: string): StraddleInfo => {
    const basePrice = symbol.includes("NIFTY50") ? 19850 : 
                     symbol.includes("NIFTYBANK") ? 44250 : 72240;
    const baseStrike = Math.round(basePrice / 50) * 50;
    const strikes = [];

    for (let i = -5; i <= 5; i++) {
      const strike = baseStrike + (i * 50);
      const callPrice = Math.max(0.5, basePrice - strike + Math.random() * 40 - 20);
      const putPrice = Math.max(0.5, strike - basePrice + Math.random() * 40 - 20);
  ...
  const atmStraddle = straddleData?.straddles.reduce((prev, current) => 
    ...
    Math.abs(prev.strike - (straddleData?.spot_price || 0)) ? current : prev
  );

  // Calculate metrics
  const totalPremium = straddleData?.straddles.reduce((sum, s) => sum + s.straddle_premium, 0) || 0;
  const avgPremium = totalPremium / (straddleData?.straddles.length || 1);
  const maxPremium = Math.max(...(straddleData?.straddles.map(s => s.straddle_premium) || [0]));
  const minPremium = Math.min(...(straddleData?.straddles.map(s => s.straddle_premium) || [0]));

  // Prepare chart data
  const chartData = straddleData?.straddles.map(s => ({
    strike: s.strike,
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <Select 
            ...
            onValueChange={(value) => setSelectedIndex(INDICES.find(i => i.value === value) || INDICES[0])}
          >
          ...
          <CardContent>
            ...
            {marketData && (
              <p className={cn(
                ...
                marketData.change >= 0 ? "text-trading-bull" : "text-trading-bear"
              )}>
                {marketData.change >= 0 ? (
              ...
              {loading ? (
                <div className="h-96 flex items-center justify-center">
                  <div className="text-center">
                    ...
                    <p className="text-sm text-muted-foreground">Loading straddle data...</p>
                  </div>
                </div>
              ) : (
                <div className="h-96">
                          ...
                          if (active && payload && payload.length) {
                            ...
                            return (
                              <div className="bg-background border border-border rounded-lg p-3 shadow-lg">
                                <p className="font-medium mb-2">Strike: {label}</p>
                                <div className="space-y-1 text-sm">
                                  <div className="flex justify-between gap-4">
                                    <span>Call Price:</span>
                                    <span className="font-mono">₹{data.call.toFixed(2)}</span>
                                  </div>
                                  <div className="flex justify-between gap-4">
                                    <span>Put Price:</span>
                                    <span className="font-mono">₹{data.put.toFixed(2)}</span>
                                  </div>
                                  <div className="flex justify-between gap-4 border-t pt-1">
                                    <span className="font-medium">Straddle:</span>
                                    <span className="font-mono font-medium">₹{data.premium.toFixed(2)}</span>
                                  </div>
                                  {data.isATM && (
                                    <div className="text-center">
                                      <Badge variant="default" className="text-xs">ATM</Badge>
                                    </div>
                                  )}
                                </div>
                              </div>
                ...
                <Table>
                  ...
                  <TableBody>
                    {straddleData?.straddles.map((straddle) => {
                      const isATM = Math.abs(straddle.strike - (straddleData?.spot_price || 0)) < 25;
                      const isITM = straddle.strike < (straddleData?.spot_price || 0);
                      
                      return (
                        <TableRow key={straddle.strike} className={isATM ? "bg-accent/50" : ""}>
                          <TableCell className="font-medium">
                            {straddle.strike}
                            {isATM && <Badge variant="default" className="ml-2 text-xs">ATM</Badge>}
                          </TableCell>
                          ...
                          <TableCell>
                            <Badge variant={isATM ? "default" : isITM ? "secondary" : "outline"}>
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="p-3 border rounded-lg">
                    <div className="font-medium text-sm mb-1">Highest Premium</div>
                  ...
                  <div className="p-3 border rounded-lg">
                    <div className="font-medium text-sm mb-1">Lowest Premium</div>
                    <div className="text-lg font-bold">
                  ...
                  <div className="p-3 border rounded-lg">
                    <div className="font-medium text-sm mb-1">Premium Spread</div>
                    <div className="text-lg font-bold">
                  ...
                  {atmStraddle && (
                    <div className="p-3 border rounded-lg bg-accent/20">
                      <div className="font-medium text-sm mb-1">ATM Breakevens</div>
                      <div className="space-y-1">
                        <div className="text-sm">
                          Upper: <span className="font-mono">₹{(atmStraddle.strike + atmStraddle.straddle_premium).toFixed(2)}</span>
                        </div>
                        <div className="text-sm">
                          Lower: <span className="font-mono">₹{(atmStraddle.strike - atmStraddle.straddle_premium).toFixed(2)}</span>
                        </div>
                      </div>
                    </div>
                  )}
