...
interface AlertContextType {
  ...
  addLogicAlert: (alert: Omit<LogicAlert, 'id' | 'createdAt' | 'isTriggered'>) => void;
...
export function AlertProvider({ children }: AlertProviderProps) {
  ...
  const [triggeredAlerts, setTriggeredAlerts] = useState<(PriceAlert | LogicAlert)[]>([]);
  ...
  const addPriceAlert = (alertData: Omit<PriceAlert, 'id' | 'createdAt' | 'isTriggered'>) => {
  ...
  const addLogicAlert = (alertData: Omit<LogicAlert, 'id' | 'createdAt' | 'isTriggered'>) => {
  ...
  const toggleAlert = (id: string, type: 'price' | 'logic') => {
    if (type === 'price') {
      ...
      setLogicAlerts(prev => prev.map(alert => 
        alert.id === id ? { ...alert, isActive: !alert.isActive } : alert
      ));
    }
  };
